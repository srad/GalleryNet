...
impl GroupFacesUseCase {
    ...
    pub async fn execute(&self, threshold: f32) -> Result<Vec<FaceGroup>, DomainError> {
        ...
        // Debug logging for similarity distribution
        let max_dot = Arc::new(std::sync::Mutex::new(0.0f32));
        ...
        let edges: Vec<(usize, usize)> = (0..n)
            ...
            .flat_map_iter(|i| {
                ...
                for j in (i + 1)..n {
                    ...
                    let sim = dot(vec_i, vec_j);
                    
                    // Sample logging: log first 10 pairs
                    let count = pair_count.fetch_add(1, Ordering::Relaxed);
                    if count < 10 {
                        tracing::info!("Face pair sample {}/{}: similarity = {:.4}", i, j, sim);
                    }
                    
                    if sim > 0.0 {
                        let mut max = max_dot.lock().unwrap();
                        if sim > *max {
                            *max = sim;
                        }
                    }
        ...
        tracing::info!("Face Clustering Debug: Loaded {} faces. Max similarity found: {:.4}. Threshold: {}. Total edges: {}", 
            n, *max_dot.lock().unwrap(), min_dot, edges.len());
        ...
        // 4. Create/Merge people from clusters
        self.repo.assign_people_to_clusters()?;

        // 5. Return grouped items
...
mod tests {
    ...
    async fn test_group_faces_use_case() {
        ...
        for id in &[id1, id2] {
            let media = MediaItem {
                ...
                exif_json: None, is_favorite: false,
                faces_scanned: false,
                tags: vec![],
                faces: vec![],
            };
    ...
    async fn test_group_faces_dissimilar() {
        ...
        repo.save_faces(id1, &[crate::domain::Face {
            id: Uuid::new_v4(), media_id: id1, box_x1: 0, box_y1: 0, box_x2: 10, box_y2: 10, cluster_id: None, person_id: None
        }], &[vec![1.0; 512]]).unwrap();
        ...
        repo.save_faces(id2, &[crate::domain::Face {
            id: Uuid::new_v4(), media_id: id2, box_x1: 0, box_y1: 0, box_x2: 10, box_y2: 10, cluster_id: None, person_id: None
        }], &[vec![-1.0; 512]]).unwrap();
    ...
    async fn test_group_faces_transitivity() {
        ...
        for i in 0..3 {
            ...
            repo.save_faces(ids[i], &[crate::domain::Face {
                id: Uuid::new_v4(), media_id: ids[i], box_x1: 0, box_y1: 0, box_x2: 10, box_y2: 10, cluster_id: None, person_id: None
