...
use application::{UploadMediaUseCase, SearchSimilarUseCase, ListMediaUseCase, DeleteMediaUseCase, GroupMediaUseCase, GroupFacesUseCase, TagLearningUseCase, FixThumbnailsUseCase, ExternalSearchUseCase, ScanFacesUseCase};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let scan_faces_use_case = Arc::new(ScanFacesUseCase::new(
        repo.clone(),
        ai.clone(),
        upload_dir.clone(),
    ));
